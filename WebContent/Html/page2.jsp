<!doctype html>
<%@page import="com.aman.NcdServlet1"%>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>ncd risk assessment</title>
    
<body> 
 </head>
  <body>
   <div class="container-fluid">
<h2>


<% 
out.println("<pre>");
out.println("patient id :"+NcdServlet1.id);

out.println("</pre>");
%>

</h2>
  <form method = "post" action = "/NcdAssessment/NcdServlet2" >
  <center>
  <h1 style="color:red" >NCD Risk Assessment Checklist</h1>
  </center>
    
    <table>
      <tr class="myTable">
        <td>Question</td>
        <td>Range</td>
        <td>Circle Any</td>
        <td>Score</td>
      </tr>

      <tr>
        <td>
          <h3>1. What is your age? (in complete years)</h3>
        </td>
        <td>
          <table class="myTable" width="100%" border="dotted dashed solid double">
            <tr>
              <td>30-39 </td>
            </tr>
            <tr>
              <td>40-49</td>
            </tr>
            <tr>
              <td>50-59</td>
              <tr>
                <td>60</td>
              </tr>
            </table>
          </td>
          <td>
            <table class="myTable" width="100%" border="dotted dashed solid double">
              <tr>
                <td>0 </td>
              </tr>
              <tr>
                <td>1</td>
              </tr>
              <tr>
              <td>2</td>
            </tr>
            <tr>
              <td>3</td>
            </tr>
          </table>
        </td>
        <td>
          
          
          <div>
            <label>input age </label><br>
            <input type="number" id="age" name="age" />
            
            
            
          </div>
        </td>
        
        
        
      </tr>
      
      
      <tr>
        <td>
          <h3>2.Do you smoke or Consume smokeless product like Gutka or Khaini</td></h3>
        <td>
          <table class="myTable" width="100%" border="dotted dashed solid double">
            
            <tr>
              
              <td> Never </td>
            </tr>
            <tr>
              <td> Used to consume in past/ sometime now </td>
              
            </tr>
            <tr>
              <td> Daily</td>
              
            </table>
          </td>
          <td>
            <table class="myTable" width="100%" border="dotted dashed solid double">
              <tr>
                <td>0 </td>
              </tr>
              <tr>
                <td>1</td>
              </tr>
              <tr>
                <td>2</td>
              </tr>

            </table>
          </td>
          <td>
            
           
            <div>
              <label>input smoke</label><br>
              <input type="number" id="smoke" name="smoke" />
            </td>
          </tr>
          <!-- 3rd para -->
          
          
          <tr>
            <td><h3>
              3. Do you consume alcohol daily?</h3></td>
              <td>
                <table class="myTable" width="100%" border="dotted dashed solid double">
                
                
                
                <tr>
                  
                  <td> yes </td>
                </tr>
                <tr>
                  <td> No</td>
                  
                </tr>
                
              </table>
            </td>
            <td>
          <table class="myTable" width="100%" border="dotted dashed solid double">
            <tr>
              <td>0 </td>
            </tr>
            <tr>
              <td>1</td>
            </tr>
            
          </table>
        </td>
        <td>
          
          <div>
            <label>input alcoholic</label><br>
            <input type="number" id="alcohol" name="alcohol" />
          </div>
          
          
          
        </td>
      </tr>
      
      <!-- 4th question -->
      
      <tr>
        <td><h3>4. Measurement of waist in (Cm)</h3></td>
        <td>
          <table class="myTable" width="100%" border="dotted dashed solid double">
            <tr>
              
              <td>Female</td>
              <td> Male</td>
            </tr>
            <tr>
              
              <td>80 cm or less</td>
              <td> 90 cm or less</td>
            </tr>
            <tr>
              <td>81-90 cm</td>
              <td> 90-100 cm</td>
            </tr>
            <tr>
              
              <td>More than 90 cm</td>
              <td> More than 100 cm</td>
            </tr>
            
          </table>
        </td>
        <td>
          <table class="myTable" width="100%" height="100%" border="dotted dashed solid double">
            <tr>
              <td>0 </td>
            </tr>
            <tr>
              <td>1</td>
            </tr>
            <tr>
              <td>2</td>
            </tr>
            <tr>
              <td>3</td>
            </tr>
            
            
          </table>
        </td>
        <td>
          
         
          <div>
           <!--<label>GENDER</label>
            Male<input type="radio" name="male"> FeMale<input type="radio" name="female">
            <br>-->
            <label>input measurement</label><br>
            <input type="number" id="measurement" name="measurement" autocomplete="off" />
            
          </div>
        </td>
      </tr>
      
      
      <!-- 5th para -->
      <tr>
        <td><h3>5. Do you undertake any physical activities for a minimum of 150 minutes a week?
        </h3></td>
        <td>
          <table class="myTable" width="100%" border="dotted dashed solid double">
            <tr>
              <td> At least 150 minutes a week
              </td>
            </tr>
            <tr>
              <td> Less than 150 min a week</td>
            </tr>
          </table>
        </td>
        <td>
          <table class="myTable" width="100%" height="100%" border="dotted dashed solid double">
            <tr>
              <td>0 </td>
            </tr>
            <tr>
              <td>1</td>
            </tr>
            
          </table>
        </td>
        <td>
          
         
          <div>
            <label>input physical</label><br>
            <input type="number" id="physical" name="physical" autocomplete="off" />
            
          </div>
        </td>
      </tr>
      
      <!-- 6thpara -->
      
      <tr>
        <td><h3>6. Do you have a family history (any one of your parents or siblings) of high blood pressure, diabetes
          and
          heart
          disease?
        </h3>
        </td>
        <td>
          <table class="myTable" width="100%" border="dotted dashed solid double">
            <tr>
              <td> yes </td>
            </tr>
            <tr>
              <td> No</td>
              
            </tr>
          </table>
        </td>
        <td>
          <table class="myTable" width="100%" height="100%" border="dotted dashed solid double">
            <tr>
              <td>0 </td>
            </tr>
            <tr>
              <td>1</td>
            </tr>
            
          </table>
        </td>
        <td>
          <div>
            <label>input Family history</label><br>
            <input type="number" id="history" name="history" autocomplete="off" />
            
          </div>
        </td>
        
      </tr>
      
    </table>
   
    <!--<button type="submit" class="btn btn-primary">Submit</button>-->
    <input type = "submit" value = "submit">
    <button type="reset" value="reset">Clear</button>
  </form>
  
</div>
<br>
  <br>
  <br>
  <h3> A score above 4 indicates that the person may be at higher risk of NCDs and needs to be prioritized for
    attending screening for following NCDs and then confirmed if there is any of them.
    Diabetes
    Hypertension
    Breast Cancer
    Cervical Cancer
    Oral Cancer
    For those who are curious - more about NCD </h3>
    <a href="https://www.who.int/news-room/fact-sheets/detail/noncommunicable-diseases ">click here</a>
    
    <!--<p>If you click the "Submit" button, the form-data will be sent to a page called "/submit".</p>-->
    

    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.2.2/dist/js/bootstrap.bundle.min.js" integrity="sha384-OERcA2EqjJCMA+/3y+gxIOqMEjwtxJY7qPCqsdltbNJuaOe923+mo//f6V8Qbsw3" crossorigin="anonymous"></script>
  
</body>
</html>